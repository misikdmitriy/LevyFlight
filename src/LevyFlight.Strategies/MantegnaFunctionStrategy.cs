...
namespace LevyFlight.Strategies
{
    public class MantegnaFunctionStrategy : IFunctionStrategy
    {
        public double Apply(double[] arguments)
        {
            ...
            sigmaX = Gamma(arguments[0] + 1) * Math.Sin(Math.PI * arguments[0] / 2);
            var divider = Gamma(arguments[0] / 2) * arguments[0] * Math.Pow(2.0, (arguments[0] - 1) / 2);
            sigmaX /= divider;
            sigmaX = Math.Pow(sigmaX.Magnitude, 1.0 / arguments[0]);

            x = GaussianRandom(0, sigmaX.Magnitude);
            y = Math.Abs(GaussianRandom(0, 1.0));

            return x / Math.Pow(y, 1.0 / arguments[0]);
        }
